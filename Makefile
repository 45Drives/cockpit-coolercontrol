...
# 	yarn --cwd $* run build $(BUILD_FLAGS)
	mkdir -p $*/dist
	cp -a $*/index.html $*/*.js $*/manifest.json $*/*.css $*/dist/
...
system-files-install:
	-cp -dR --preserve=mode system_files/* $(DESTDIR)/

system-files-install-local:
	-cp -dR --preserve=mode system_files/* $(DESTDIR)/

system-files-install-remote:
	-rsync -rlptvh system_files/* $(REMOTE_TEST_USER)@$(REMOTE_TEST_HOST):$(DESTDIR)/
